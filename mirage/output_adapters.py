...
class SpatialOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        assert self.patch_size_full is not None
        ...
        print(f"Building 2D sincos posemb with h={h_posemb} and w={w_posemb}")
        ...
        for task, info in input_info["tasks"].items():
            ...
            if info['has_posemb']:
                ...
                assert info['num_tokens'] == pos_emb.shape[1], \
                    f"# tokens ({info['num_tokens']}) != # pos. embeddings ({pos_emb.shape[1]})"
    ...
    def get_queries_and_context(self, context_tokens, input_info, ids_keep, ids_restore):
        ...
        if self.use_task_queries and self.task in input_info['tasks']:
            ...
            if self.task is not None and self.task_embeddings is not None and self.task in self.task_embeddings:
...
class Adapter(nn.Module):
    def __init__(self, main_tasks: Union[tuple, list] = ('bscan',)):
        super().__init__()
...
class SegmenterMaskTransformerAdapter(Adapter):
